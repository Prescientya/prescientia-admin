...
<div class="ev-page">
    ...
    <form action="{{ route('events.update', $event) }}" method="POST" data-loading>
        @csrf
        @method('PUT')
                    ...
                    <div class="form-grid-2">
                        <div class="form-group form-col-full">
                            ...
                            <input type="text" name="title" class="form-control"
                                   ...
                                   value="{{ old('title', $event->title) }}" maxlength="100" required>
                            @error('title')
                        ...
                        <div class="form-group form-col-full">
                            ...
                            <textarea name="description" class="form-control" rows="4"
                                      placeholder="Tuliskan deskripsi event..." maxlength="1000">{{ old('description', $event->description) }}</textarea>
                            @error('description')
                                <span class="form-hint" style="color:#ef4444;">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group form-col-full">
                            ...
                            <input type="url" name="link" class="form-control"
                                   ...
                                   value="{{ old('link', $event->link) }}" maxlength="500">
                            @error('link')
                                <span class="form-hint" style="color:#ef4444;">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" name="release_date" class="form-control"
                                   value="{{ old('release_date', $event->release_date->format('Y-m-d')) }}" required>
                            @error('release_date')
                                <span class="form-hint" style="color:#ef4444;">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="target_audience" id="targetAudience" class="form-control" required>
                            <option value="semua"  {{ old('target_audience', $event->target_audience) === 'semua'  ? 'selected' : '' }}>Semua (Guru & Siswa)</option>
                            <option value="guru"   {{ old('target_audience', $event->target_audience) === 'guru'   ? 'selected' : '' }}>Guru Saja</option>
                            <option value="siswa"  {{ old('target_audience', $event->target_audience) === 'siswa'  ? 'selected' : '' }}>Siswa Saja</option>
                            <option value="kelas"  {{ old('target_audience', $event->target_audience) === 'kelas'  ? 'selected' : '' }}>Kelas / Jurusan Tertentu</option>
                        </select>
                    </div>

                    {{-- Conditional: Kelas Target --}}
                    <div id="targetKelasSection" class="ev-target-section"
                         style="{{ old('target_audience', $event->target_audience) === 'kelas' ? '' : 'display:none;' }}">
                            ...
                            <div class="ev-checkbox-grid">
                                ...
                                <label class="ev-checkbox-item">
                                    <input type="checkbox" name="target_classes[]" value="{{ $cls->id }}"
                                           {{ in_array($cls->id, old('target_classes', $selectedClasses)) ? 'checked' : '' }}>
                            ...
                            <div class="ev-checkbox-grid">
                                ...
                                <label class="ev-checkbox-item">
                                    <input type="checkbox" name="target_grades[]" value="{{ $grade }}"
                                           {{ in_array($grade, old('target_grades', $selectedGrades)) ? 'checked' : '' }}>
                            ...
                            <div class="ev-checkbox-grid">
                                ...
                                <label class="ev-checkbox-item">
                                    <input type="checkbox" name="target_majors[]" value="{{ $major }}"
                                           {{ in_array($major, old('target_majors', $selectedMajors)) ? 'checked' : '' }}>
                                    <span>{{ $major }}</span>
                                </label>
                                @endforeach
                            </div>
