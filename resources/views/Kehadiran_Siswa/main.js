...
(function () {
    ...
    function sourceLabel(source) {
        const map = {
            ...
            wali_kelas:    { label: 'Wali Kelas',     cls: 'source-badge--walikelas' },
            guru_pengajar: { label: 'Guru Pengajar',  cls: 'source-badge--walikelas' },
            self_report:   { label: 'Self Report',    cls: 'source-badge--manual'    },
        };
        ...
        editForm?.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (data.ok) {
                    ...
                    // Refresh data-attrs on edit + detail buttons for this row
    ...
    function initAutocomplete() {
        ...
        const kelasInput = document.getElementById('manualKelasDisplay');
        ...
        nameInput.addEventListener('input', function () {
            ...
            if (!q) {
                ...
                list.innerHTML = '';
                hiddenId.value = '';
                if (kelasInput) kelasInput.textContent = '–';
                return;
            }
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(s => {
                            ...
                            div.innerHTML = `
                                ...
                                <span class="autocomplete-item__meta">NIS ${escHtml(s.nis ?? '–')} &bull; Kelas ${escHtml(s.kelas)}</span>
                            `;
                            div.addEventListener('mousedown', (e) => {
                                e.preventDefault();
                                nameInput.value        = s.name;
                                hiddenId.value         = s.id;
                                if (kelasInput) kelasInput.textContent = s.kelas;
                                list.classList.remove('open');
                            });
...
(function () {
    ...
    function initStatusUpdate() {
        document.querySelectorAll('[data-status-btn]').forEach(btn => {
            btn.addEventListener('click', async function () {
                ...
                const row    = document.getElementById('row-' + attId);
                ...
                try {
                    const res = await fetch(`/attendance/siswa/${attId}/status`, {
                        method: 'PATCH',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': CSRF,
                    ...
                    if (data.ok) {
                        // Update badge in row
                        const badge = document.getElementById('badge-' + attId);
                        if (badge) {
                            badge.className = `att-badge att-badge--${data.status}`;
                            badge.textContent = capitalize(data.status);
                        }
                        PSC.toast('Status diperbarui', 'success');
                    }
                } catch {
                    PSC.toast('Gagal memperbarui status', 'error');
                } finally {
    ...
    /* ──────────────────────────────────────────────────────────
     | 3. EDIT WAKTU MODAL
     |─────────────────────────────────────────────────────────*/
    const timeModal    = document.getElementById('modalEditWaktu');
    const timeForm     = document.getElementById('formEditWaktu');
    const timeCheckIn  = document.getElementById('editCheckIn');
    const timeCheckOut = document.getElementById('editCheckOut');
    let   currentTimeAttId = null;

    function initTimeModal() {
        document.querySelectorAll('[data-open-time-modal]').forEach(btn => {
            btn.addEventListener('click', function () {
                const attId = this.dataset.attId;
                currentTimeAttId = attId;

                timeCheckIn.value  = this.dataset.checkIn  ?? '';
                timeCheckOut.value = this.dataset.checkOut ?? '';
        ...
        timeForm?.addEventListener('submit', async function (e) {
            e.preventDefault();
            if (!currentTimeAttId) return;

            const submitBtn = this.querySelector('[type="submit"]');
            submitBtn.disabled = true;

            try {
                const res = await fetch(`/attendance/siswa/${currentTimeAttId}/time`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CSRF,
                    ...
                    body: JSON.stringify({
                        check_in_time:  timeCheckIn.value  || null,
                        check_out_time: timeCheckOut.value || null,
                    }),
                });
                ...
                if (data.ok) {
                    const inEl  = document.getElementById('ci-' + currentTimeAttId);
                    const outEl = document.getElementById('co-' + currentTimeAttId);

                    if (inEl)  inEl.textContent  = data.check_in  || '–';
                    if (outEl) outEl.textContent  = data.check_out || '–';

                    // Update data attributes for re-open
                    const reopenBtn = document.querySelector(`[data-open-time-modal][data-att-id="${currentTimeAttId}"]`);
                    if (reopenBtn) {
                        if (data.check_in)  reopenBtn.dataset.checkIn  = data.check_in;
                        if (data.check_out) reopenBtn.dataset.checkOut = data.check_out;
                    }

                    timeModal.classList.remove('open');
                    PSC.toast('Jam absensi diperbarui', 'success');
                }
            } catch {
                PSC.toast('Gagal menyimpan waktu', 'error');
            } finally {
                submitBtn.disabled = false;
            }
        });
        ...
        nameInput.addEventListener('input', function () {
            ...
            debounce = setTimeout(async () => {
                try {
                    ...
                    if (data.length === 0) {
                        list.innerHTML = `<div class="autocomplete-empty">Tidak ada siswa yang bernama "<strong>${escHtml(q)}</strong>"</div>`;
                    } else {
                        data.forEach(s => {
                            const div = document.createElement('div');
                            div.className = 'autocomplete-item';
                            ...
                            div.addEventListener('mousedown', (e) => {
                                ...
                                // Also set the class_id select if available
                                const classSelect = document.getElementById('manualClassId');
                                if (classSelect && s.class_id) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        PSC.initOpenButtons();
        PSC.initActionDropdowns('.ka-action__btn', '.ka-dropdown');
        // Wire all .modal-close buttons to close their parent overlay
        document.querySelectorAll('.modal-close').forEach(btn => {
