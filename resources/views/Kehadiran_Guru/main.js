...
(function () {
    ...
    function initDetailModal() {
        document.querySelectorAll('[data-detail-btn]').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                this.closest('.kg-dropdown')?.classList.remove('open');

                document.getElementById('detail-name').textContent      = d.name       || '–';
                document.getElementById('detail-nip').textContent       = d.nip        || '–';
                document.getElementById('detail-dept').textContent      = d.department || '–';
    ...
    function initEditModal() {
        ...
        editForm?.addEventListener('submit', async function (e) {
            ...
            try {
                const res = await fetch(`/attendance/guru/${currentEditId}`, {
                    method: 'PATCH',
                    headers: {
                ...
                if (data.ok) {
                    ...
                    // Refresh data-attrs on action buttons
    ...
    function initDeleteModal() {
        document.querySelectorAll('[data-delete-btn]').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                document.getElementById('deleteTeacherName').textContent = this.dataset.name || '';
                this.closest('.kg-dropdown')?.classList.remove('open');
        ...
        confirmDelBtn?.addEventListener('click', async function () {
            ...
            try {
                const res = await fetch(`/attendance/guru/${currentDeleteId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': CSRF,
                        'Accept': 'application/json',
    ...
    function initAutocomplete() {
        ...
        const hiddenId   = document.getElementById('manualTeacherId');
        const deptDisp   = document.getElementById('manualDeptDisplay');
        ...
        nameInput.addEventListener('input', function () {
            ...
            if (!q) {
                ...
                if (deptDisp) deptDisp.textContent = '–';
                return;
            }

            debounce = setTimeout(async () => {
                try {
                    const res  = await fetch(`/attendance/guru/search?q=${encodeURIComponent(q)}`, {
                    ...
                    if (data.length === 0) {
                        list.innerHTML = `<div class="autocomplete-empty">Tidak ada guru bernama "<strong>${escHtml(q)}</strong>"</div>`;
                    } else {
                        data.forEach(t => {
                            const div = document.createElement('div');
                            div.className = 'autocomplete-item';
                            div.innerHTML = `
                                <span class="autocomplete-item__name">${escHtml(t.name)}</span>
                                <span class="autocomplete-item__meta">NIP ${escHtml(t.nip ?? '–')} &bull; ${escHtml(t.department)}</span>
                            `;
                            div.addEventListener('mousedown', (e) => {
                                e.preventDefault();
                                nameInput.value = t.name;
                                if (hiddenId) hiddenId.value = t.id;
                                if (deptDisp) deptDisp.textContent = t.department || '–';
                                list.classList.remove('open');
                            });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        PSC.initModalClose();
        PSC.initOpenButtons();
        PSC.initActionDropdowns('.kg-action__btn', '.kg-dropdown');
